...
from thumbnailer.responses import generate_binary_response, generate_favicon_response, generate_json_respone
...
def handler(event, context):
  ...
  if not source_bucket:
    return generate_json_respone(400, f'source bucket not configured via "{MEDIA_BUCKET_ENV_KEY}"')

  thumbs_bucket = environ.get(THUMBS_BUCKET_ENV_KEY)
  if not thumbs_bucket:
    return generate_json_respone(400, f'thumbs bucket not configured via "{THUMBS_BUCKET_ENV_KEY}"')
  ...
  with NamedTemporaryFile(suffix=ext) as temp:
    try:
      ...
      info(f'{thumbs_bucket}/{thumbnail_key} not found. creating it from {source_bucket}/{key}')
